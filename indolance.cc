...
int logincustomer(int attempt)
{
    ...
    {
        printf("Maaf anda sudah 3 kali percobaan salah, silahkan ganti password \n");
        system("pause");
        system("cls");
        fclose(akun_customer);
        ganti_password();
...
void ganti_password(){
    ...
    fread(&data, sizeof(data), 1, akun);
    printf("\nUsername saat ini: %s\n", data.username);
    printf("Masukkan password baru: ");
    scanf("%s", newpass);
    strcpy(data.password, newpass);
    fseek(akun, 0, SEEK_SET);
    fwrite(&data, sizeof(data), 1, akun);
    fclose(akun);
    main();
}
